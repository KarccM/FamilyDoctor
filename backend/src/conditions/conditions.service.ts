...
export class ConditionsService {
  ...
  async create(createConditionDto: CreateConditionDto): Promise<Condition> {
    let condition = null;
    condition = await this.conditionModel.findOne({
  ...
  async findAllSymptoms(): Promise<Condition[]> {
    return await this.conditionModel.find({
      conditionType: ConditionType.Symptom,
    });
  }

  async findAllMedicalConditions(): Promise<Condition[]> {
    return await this.conditionModel.find({
      conditionType: ConditionType.MedicalCondition,
    });
  }

  async findAllPatientInfo(): Promise<Condition[]> {
    return await this.conditionModel.find({
      conditionType: ConditionType.PatientInfo,
    });
  }
  ...
  async update(
    ...
    try {
      ...
      if (condition) {
        ...
        if (updateConditionDto?.question)
          condition.question = updateConditionDto.question;
        if (updateConditionDto?.conditionValuesType)
          condition.conditionValuesType =
            updateConditionDto.conditionValuesType;
        if (updateConditionDto?.values)
          condition.values = updateConditionDto.values;
      }
    } catch (error) {
