...
export abstract class Conclusion{
    ...
    topNRules(): Rule[]{
        return this.rules.sort((ruleA, ruleB)=> compareFn(ruleA.ruleWeight, ruleB.ruleWeight, 'asc'));
    }
...
export class Node{
    ...
    toString(){
        return `\n[Node] with conclusion ${this.conclusion}\n \t with score ${this.getScore()} \t and previouslyTried = ${this.previouslyTried}\n`
    }
}
...
let node2 = new Node(new Disease('2كريب',[
  ...
  ], 99, ['treatment'], 'specialist', ['notes']));
  
let node3 = new Node(new Disease('3كريب', [
    new Rule([vomit, nausea, bloating]),
    new Rule([stuffyNose, hearingImpairment, bloating]),
    new Rule([diarrhea, nausea, hearingImpairment]),
  ], -33, ['treatment'], 'specialist', ['notes']));
