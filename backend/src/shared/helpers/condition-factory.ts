...
export function ConditionFactory(condition: Condition): ConditionClass {
    ...
    switch(condType){
        case ConditionType.Symptom:
            condInstance = new Symptom(condition.name, condition.question, condition.values, condition.conditionValuesType);
            condInstance.value = condition.value != null ? condition.value : null
            break;
        case ConditionType.MedicalCondition:
            condInstance = new MedicalCondition(condition.name, condition.question, condition.values, condition.conditionValuesType);
            condInstance.value = condition.value != null ? condition.value : null
            break;
        case ConditionType.PatientInfo:
            condInstance = new PatientInfo(condition.name, condition.question, condition.values, condition.conditionValuesType);
            condInstance.value = condition.value != null ? condition.value : null
            break;
        default:
