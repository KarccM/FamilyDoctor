...
export function ConditionFactory(condition: ConditionValue): ConditionClass {
    ...
    switch(condType){
        ...
        case ConditionType.MedicalCondition:
            condInstance = new MedicalCondition(condition['v'].name, condition['c'].question, condition['c'].values);
            break;
        case ConditionType.PatientInfo:
            condInstance = new PatientInfo(condition['c'].name, condition['c'].question, condition['c'].values);
            break;
        default:
            condInstance = new ConditionClass(condition['c'].name, condition['c'].question, condition['c'].values);
            break;
    }
