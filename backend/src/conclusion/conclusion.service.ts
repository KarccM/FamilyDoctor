...
export class ConclusionService {
  ...
  async findAll(): Promise<Conclusion[]> {
    return await this.conclusionModel.find({});
  }

  async findAllDiagnosis(): Promise<Conclusion[]> {
    return await this.conclusionModel.discriminators[ConclusionType.Diagnosis].find({});
  }

  async findOne(id: string): Promise<Conclusion> {
    return await this.conclusionModel.findById({});
  }
