...
export class ChatsService {
  ...
  async answer(answerChatDto: AnswerChatDto): Promise<ChatResponse> {
    ...
    try {
      chat = await this.chatModel.findOne({
        _id: answerChatDto._id,
        user_id: answerChatDto.user_id,
      });
      if (chat == null || chat == undefined) {
        throw new BadRequestException(
          `Chat with id:${answerChatDto._id} does not exist`,
        );
      }
      let condition = await this.conditionsService.findOneByName(
        answerChatDto.condition.name,
      );
      if (condition == null || condition == undefined) {
        throw new BadRequestException(
          `Condition ${answerChatDto.condition.name} does not exist`,
        );
      }
      ...
      pqueue
        .getHead()[0]
        .conclusion.rules.forEach((r) => console.log(r.conditions));
    } catch (error) {
      throw error;
    }
    await chat.save();
